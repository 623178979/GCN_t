...
def learn(args,network='gnn',
        ...
        #   save_path = './GCN_t/model',
          save_path = None,
        #   load_path = None,
          load_path = '/root/autodl-tmp/GCN_t/bflesskan_20_10sps',
          total_time = 20,
          ):
    # total_time = 2000
    batch_sample = 25
    batch_sample_eval = 25#10
    eval_val = 0
    time_limit = 10
    exploration_strategy = 'relpscost'

    instances_valid = []
    instances_train = glob.glob('/root/autodl-tmp/GCN_t/smallatc/train/*.mps')
    instances_valid += ['/root/autodl-tmp/GCN_t/smallatc/test/instances{}.mps'.format(i+1) for i in range(50)]
    out_dir = '/root/autodl-tmp/test_par'
    ...
    agent = DDPG(actor_critic=actor_critic,batch_size=batch_size,critic_l2_reg=critic_l2_reg,pi_lr=actor_lr,q_lr=critic_lr,tau=tau,gamma=gamma,replay_size=50)
    ...
    for epoch in range(1):
        ...
        result = 'test_step'+str(nb_eval_steps)+'s.csv'
        os.makedirs('/root/autodl-tmp/GCN_t/bflesskan_20_10sps/test_results', exist_ok=True)
        with open("/root/autodl-tmp/GCN_t/bflesskan_20_10sps/test_results/{}".format(result),'w', newline='') as csvfile:
            ...
            for cycle in range(1):
                ...
                if cycle%1==0:
                    ...
                    for cyc in range(len(instances_valid)//batch_sample_eval):
                        ...
                        print('init_sol time:',time.time()-a_1)
                        init_sols = ini_sol
                        ...
                        pre_sols = np.zeros([2, batch_sample_eval, 11590])
                        ...
                        for t_rollout in range(nb_eval_steps):
                            ...
                            # total_time = each_time * batch_sample_eval
                            if time.time()-a_1 > total_time:
                                print('break')
                                break
...
if __name__ == '__main__':
    ...
    step_list = [1,2,6,7,8,9,10,20,50,100]

    # for t in time_list:
    #     for step in step_list:
            # learn(args=arg,total_time=t,nb_eval_steps=step)
    learn(args=arg)