...
def generate_atc(file_name):
    while 1:
        ...
        # print(b_g)
        sys = LinearSystem(A,B,C,D,noise_scale=1)
        ...
        gamma_o12 = sbc.outside_rectangle_formula(OBSTACLE1,5,6,n)
        gamma_o21 = sbc.outside_rectangle_formula(OBSTACLE2,0,1,n)
        gamma_o22 = sbc.outside_rectangle_formula(OBSTACLE2,5,6,n)
        gamma_o31 = sbc.outside_rectangle_formula(OBSTACLE3,0,1,n)
        gamma_o32 = sbc.outside_rectangle_formula(OBSTACLE3,5,6,n)

        gamma_b1 = sbc.inside_rectangle_formula(BACKGROUND,0,1,n)
        gamma_b2 = sbc.inside_rectangle_formula(BACKGROUND,5,6,n)
        ...
        gamma_g2 = sbc.inside_rectangle_formula(GOAL,5,6,n)
        gamma_g2_t1 = sbc.outside_rectangle_formula(RUNWAY,5,6,n)
        ...
        d = ssp.LinearPredicate(a=[-1,-1,0,0,0,1,1,0,0,0],b=b_g['d_safe'])
        ...
        Q = np.zeros([n,n])
        R = np.eye(n)
        x0 = np.array([b_g['flight_1'][0,0],b_g['flight_1'][1,0],0,0,0,b_g['flight_2'][0,0],b_g['flight_2'][1,0],0,0,0])
        # print(x0)
        u_limit = 200
        u_limit_angle = math.pi/2
        u_min = np.array([-u_limit, -u_limit, 0, -u_limit_angle, 0, -u_limit, -u_limit, 0, -u_limit_angle, 0])
        u_max = np.array([u_limit, u_limit, 0, u_limit_angle, 0, u_limit, u_limit, 0, u_limit_angle, 0])

        solver = MICPSolver(var_phi, sys, x0, b_g['T_2'], robustness_cost=True)
...
for i in range(10):
    file_names = []
    for j in range(10):
        file_names.append('./GCN_t/atcdata/test/instances{}.mps'.format(10*i+j+1))
    print(file_names)
...
# file_names = []
# for i in range(10):
#     file_names.append('./GCN_t/atcdata/validation/instances{}.mps'.format(i+1))
